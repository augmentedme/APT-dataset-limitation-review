'''
This script calculate performance metrics with various ML algorithms on Unraveled dataset
'''
...
def performance_evaluation(hof, X_test, train_time):
    start_time = datetime.datetime.now()
    y_pred = predicter(hof[0], X_test)
    end_time = datetime.datetime.now()
    pred_time = end_time - start_time

    b_acc, cm, f1_score, tnr, fpr, train_time, pred_time = calculate_metrics(y_test, y_pred, train_time, pred_time)

    return b_acc, cm, f1_score, tnr, fpr, train_time, pred_time
