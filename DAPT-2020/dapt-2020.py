...
def clean_data(df):
    ...
    # list of columns to drop. some column has zero values only and others are removed according to the original datatset paper.
    drop_cols = [0, 1, 2, 3, 6, 36, 38, 39, 54, 61, 62, 63, 64, 65, 66, 71, 74]
...
def load_data(file_name_train, file_name_test):
    ...
    column_names = df_train.columns[:-2].tolist()
...
def csv_result(dt_acc, svm_acc, gnb_acc, rf_acc, knn_acc, mlp_acc, stdev_mlp):
    if os.path.exists('Bal_Acc.csv'):
        print()
    else:
        with open('Bal_Acc.csv', 'a+', newline='') as f:
            header = ['dt_acc', 'svm_acc', 'gnb_acc', 'rf_acc', 'knn_acc', 'mlp_acc', 'stdev_mlp']
            filewriter = csv.DictWriter(f, fieldnames=header)
            filewriter.writeheader()
    row_contents = [str(dt_acc), str(svm_acc), str(gnb_acc), str(rf_acc), str(knn_acc), str(mlp_acc), str(stdev_mlp)]
...
if __name__ == "__main__":
    ...
    # Generating balanced accuracy without new features
    ...
    csv_result(dt_acc, svm_acc, gnb_acc, rf_acc, knn_acc, mlp_acc, stdev_mlp)
